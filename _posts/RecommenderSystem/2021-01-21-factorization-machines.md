...
지금까지 다룬 모델은 2-way (degree = 2)인 Factorization Machine을 다루었다. degree가 2라는 것은 Latent Factor 조합의 후보를 2개로 하겠다는 의미이다.
...
Matrix Factorization은 추천 시스템에서 전통적으로 사용되는 모델이다. Matrix Factorization은 두개의 categorical 변수를 factorize하는데 추천시스템에서는 Rating Matrix를 User latent matrix와 Item latent matrix로 factorize하게 된다. Factorization Machine의 parameter를 적절히 조절하면 좋은 성과를 낸 Matrix Factorization 기반의 모델을 따라할 수 있다.
